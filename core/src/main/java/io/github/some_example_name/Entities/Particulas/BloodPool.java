...
public class BloodPool {
    ...
    public void update(float delta, Vector2 cameraPosition, float distanceThreshold) {
        ...
        if (growthProgress < 1f) {
            growthProgress += delta * 3f; // Leva ~0.33 segundos para crescer
    ...
    public void render(SpriteBatch batch, TextureRegion[] poolSprites, 
        ...
        if (!isActive || alpha < 0.05f) return;
        ...
        float rippleOffset = (float)Math.sin(rippleTimer) * rippleIntensity * 2f;
        float rippleScale = 1f + (float)Math.sin(rippleTimer * 1.5f) * 0.05f;
        ...
        // Desenha sombra sutil
        batch.setColor(0.2f, 0.1f, 0.1f, alpha * 0.3f);
        batch.draw(
            sprite,
            screenX - renderSize / 2f + 2f,
            screenY - renderSize / 4f - 1f,
            renderSize * 0.9f * rippleScale,
            renderSize * 0.45f * rippleScale
        );
        
        // Desenha poça principal
        batch.setColor(r, g, b, alpha);
        batch.draw(
            sprite,
            screenX - renderSize / 2f + rippleOffset,
            screenY - renderSize / 4f,
            renderSize * rippleScale,
            renderSize * 0.5f * rippleScale
        );
        
        // Brilho para poças frescas
        if (dryness < 0.3f && alpha > 0.7f) {
            float glowAlpha = alpha * 0.4f * (1f - dryness);
            batch.setColor(1f, 0.3f, 0.2f, glowAlpha);
            
            float glowSize = renderSize * 1.1f;
            batch.draw(
                sprite,
                screenX - glowSize / 2.2f,
                screenY - glowSize / 4.5f,
                glowSize,
                glowSize * 0.55f
            );
        }
