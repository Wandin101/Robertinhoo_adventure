...
public class SistemaLuz {
    ...
    private static final Color DOOR_SKULL_COLOR_PRIMARY = new Color(0.886f, 1.0f, 0.290f, 1f); // #e2ff4a
    private static final Color DOOR_SKULL_COLOR_SECUNDARIA = new Color(0.4f, 0.95f, 0.5f, 1f); // #66ff80
    private static final Color DOOR_SKULL_COLOR_QUENTE = new Color(0.95f, 0.85f, 0.4f, 1f); // Tom quente
    ...
    public void renderDoorSkullLight(float x, float y, float baseRadius, float delta) {
        float elapsed = (TimeUtils.millis() - startTime) * 0.001f;

        float pulseLento = MathUtils.sin(elapsed * 1.8f) * 0.03f + 1f; // Muito lento
        float pulseMedio = MathUtils.sin(elapsed * 3.0f + 1f) * 0.02f + 1f; // Suave
        float pulseSuave = MathUtils.sin(elapsed * 5f + 2f) * 0.01f + 1f; // Quase imperceptível

        float combinedPulse = (pulseLento + pulseMedio + pulseSuave) / 3f;
        float currentRadius = baseRadius * combinedPulse;

        // ✅ VARIAÇÃO DE COR SUAVE
        float colorMix = (MathUtils.sin(elapsed * 0.8f) * 0.2f + 0.5f); // Transição bem lenta

        Color currentColor = new Color();

        if (colorMix < 0.5f) {
            float t = colorMix * 2f;
            currentColor.r = MathUtils.lerp(DOOR_SKULL_COLOR_PRIMARY.r, DOOR_SKULL_COLOR_QUENTE.r, t);
            currentColor.g = MathUtils.lerp(DOOR_SKULL_COLOR_PRIMARY.g, DOOR_SKULL_COLOR_QUENTE.g, t);
            currentColor.b = MathUtils.lerp(DOOR_SKULL_COLOR_PRIMARY.b, DOOR_SKULL_COLOR_QUENTE.b, t);
        } else {
            float t = (colorMix - 0.5f) * 2f;
            currentColor.r = MathUtils.lerp(DOOR_SKULL_COLOR_QUENTE.r, DOOR_SKULL_COLOR_SECUNDARIA.r, t);
            currentColor.g = MathUtils.lerp(DOOR_SKULL_COLOR_QUENTE.g, DOOR_SKULL_COLOR_SECUNDARIA.g, t);
            currentColor.b = MathUtils.lerp(DOOR_SKULL_COLOR_QUENTE.b, DOOR_SKULL_COLOR_SECUNDARIA.b, t);
        }

        float alphaVariation = (MathUtils.sin(elapsed * 1.2f) * 0.03f + 0.97f);
        currentColor.a = 0.6f * alphaVariation;

        // ✅ MOVIMENTO MÍNIMO
        float offsetX = MathUtils.sin(elapsed * 3f) * 0.5f;
        float offsetY = MathUtils.cos(elapsed * 2f) * 0.3f;

        // ✅ CAMADA 1: LUZ PRINCIPAL DA CAVEIRA
        renderLight(x + offsetX, y + offsetY, currentRadius, currentColor);

        // ✅ CAMADA 2: NÚCLEO MAIS PEQUENO
        Color coreColor = new Color(
                MathUtils.lerp(DOOR_SKULL_COLOR_PRIMARY.r, DOOR_SKULL_COLOR_SECUNDARIA.r, 0.5f),
                MathUtils.lerp(DOOR_SKULL_COLOR_PRIMARY.g, DOOR_SKULL_COLOR_SECUNDARIA.g, 0.5f),
                MathUtils.lerp(DOOR_SKULL_COLOR_PRIMARY.b, DOOR_SKULL_COLOR_SECUNDARIA.b, 0.5f),
                0.1f // Alpha bem baixo
        );

        float corePulse = MathUtils.sin(elapsed * 4f) * 0.05f + 0.95f;
        float coreRadius = baseRadius * 0.1f * corePulse;
        renderLight(x + offsetX * 0.1f, y + offsetY * 0.1f, coreRadius, coreColor);

        // ✅ CAMADA 3: HALO EXTERNO (muito suave)
        Color haloColor = new Color(DOOR_SKULL_COLOR_SECUNDARIA);
        float haloRadius = baseRadius * 1.1f;
        haloColor.a = 0.08f; // Alpha muito baixo
        renderLight(x, y, haloRadius, haloColor);
    }
